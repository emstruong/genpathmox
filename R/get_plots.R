...
#'
#' @references Lamberti, G. et al. (2016). The Pathmox approach for PLS path modeling 
...
#' the endogenous latent variables associated with the terminal nodes. 
#' The user indicates the endogenous latent variable they want to visualize. 
...
#' 
#' @param x An object of the class \code{"plstree"}
#' @param LV A string indicating the name of the endogenous latent variable. The label
...
{	
  ...
  if(bycoef)
  {
    end_eq=t(x$terminal_paths[grepl(chars, nn, fixed = TRUE),])
  ...
  for (n in 1:nn) 
  {
    if(bycoef){
      ...
      barplot(end_eq[,n], main = paste(nodes.name[n]),col=c("#99CCCC","#009999")[maxfac],cex.names=cex.names, 
              cex.axis=cex.axis,cex.main=cex.main,...)
      
    }
    else{
      barplot(end_eq[,n], main = paste(nodes.name[n],"\n","R^2:",rq[n]),col = colors,cex.names=cex.names, 
              cex.axis=cex.axis,cex.main=cex.main,...)
...
#' Which matters most for which graduate students? \emph{Quality and Quantity}, 1-22.
