...
#' @param .signif A decimal value indicating the minimum threshold significance 
...
#' @param .size  A decimal value indicating the minimum admissible size for a node. It is 
#' equal to 0.10 by default
#' @param .size_candidate An integer indicating the minimum admissible size for a candidate 
...
#' 
#' @references Hair, J. F. et al. (2019). When to use and how to report the results of PLS-SEM.  
#' \emph{European business review}, doi: 10.1108/EBR-11-2018-0203.
#' 
#' @references Hair Jr, J. F., et al. (2017). \emph{Advanced issues in partial least squares structural 
#' equation modeling}. SAGE publications.
#' 
#' @references Henseler, J., et al. (2016). Testing measurement invariance of composites using 
...
#' 
#' @references Lohmoller, J.-B. (1989). \emph{Latent Variable Path Modeling with Partial Least 
#' Squares}, Physica: Heidelberg.
#'
#' @references Rademaker M. E. (2020). cSEM: Composite-Based Structural Equation Modeling.
#' Available at \url{https://CRAN.R-project.org/package=cSEM}
#' 
#' @references Ringle, C. M. et al. (2015). SmartPLS 3. Boenningstedt: SmartPLS. Retrieved 
#' from  \url{https://www.smartpls.com}
#'
#' @import stats diagram methods graphics grDevices
  ...
  
{
  ...
  check = check_arg_mox(.data,.catvar,.signif,.size,.deep)
  .data = check$x
  .catvar = check$y
  .signif = check$signif
  .size = check$size
  ...
  info.mox(.signif,.size,.deep,.catvar)
  
  par_mode	= list(signif=.signif,size=.size,deep=.deep,data =.data,catvar=.catvar,
                  inner=inner,outer=outer,mode=mode,scheme=.scheme)
  ...
  while(length(new_nodes) > 0){
    ...
    if(length(n@elements)>=min.ind.node$min.n.ind && showDeepth(n) < .deep){
      ...
      tmp = partopt(d,s,inner,outer,mode,.scheme,.size_candidate)
      ...
      if(tmp$pvl.opt <= .signif && any(!is.null(tmp$pvl.opt)) == TRUE) {
        ...
        mod=test.partition(d,inner,outer,mode,.scheme,modtwo,.signif)
        
        if(any(mod$pvc <= .signif) == TRUE){
  ...
  else
  {
    ...
    for(i in 2:length(terminal))
    {
      ...
      pls_model=pls(dt_terminal, inner, outer, mode)$path
